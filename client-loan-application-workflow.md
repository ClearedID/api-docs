...
- **Body** (VerificationStatus style): includes `customerName`, `verifications` (array with `type`, `status`, and type-specific fields — identity rows may include `documentType`, `clearedAt`, `expiresAt`, **`taxNumber`**, **`idNumber`**, **`dateOfBirth`**, `initialReviewStatus`), and **`onboarding`**: `{ pageId, onboardingPageId, urlParameters }`, plus **`eventName`** (e.g. `identityVerificationStarted`, `verificationRequestApproved`, `identityVerificationCleared`, `identityVerificationRejected`, `initialReviewCompleted`, `dueDiligenceStarted`; default `verificationStatusUpdated`), **`eventContext`** (cleared / IR contexts may also carry `taxNumber` / `idNumber` / `dateOfBirth` when known), **`eventOccurredAt`** (ISO). Full field rules: [Onboarding webhooks](./onboarding/onboarding-webhooks.md).
